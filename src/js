...
const STORAGE_KEY = "adlis_conversations_v3";
...
//  COMPOSER
// ─────────────────────────────────────────────
function Composer({input,setInput,onSend,isLoading,t,attachments,onAddAttachment,onRemoveAttachment}){
...
//  GEMINI API CALL
// ─────────────────────────────────────────────
async function callGemini({history, currentAttachments, content, onStream}){
  const apiKey = process.env.REACT_APP_GEMINI_API_KEY;
  if(!apiKey) throw new Error("مفتاح Gemini API غير موجود — أضف REACT_APP_GEMINI_API_KEY في Netlify");
  ...
  for(const att of currentAttachments){
    if(att.type === "image"){
      userParts.push({ inlineData: { mimeType: att.mediaType, data: att.base64 }});
    }
  }
  if(content) userParts.push({ text: content });
  if(!userParts.length) userParts.push({ text: "(ملف مرفق)" });

  contents.push({ role: "user", parts: userParts });

  const body = {
    systemInstruction: { parts: [{ text: ADLIS_SYSTEM }] },
    contents,
    generationConfig: {
...
function useChatCore(){
  ...
  const sendMessage=useCallback(async()=>{
    ...
    setInput("");setAttachments([]);setIsLoading(true);setStreamingContent("");
    ...
    try{
      ...
      setConversations(prev=>prev.map(c=>c.id===convId
        ?{...c,messages:[...c.messages,{id:Date.now().toString(),role:"assistant",content:fullText}]}
        :c
      ));
      setStreamingContent("");
    }catch(err){
  ...
  },[input,attachments,isLoading,activeId,conversations]);
  ...
  return{
    ...
    isLoading,streamingContent,messagesEndRef,activeConv,
          ...
          {(!activeConv||activeConv.messages.length===0)&&!isLoading?(
            ...
            <>
              ...
              {isLoading&&streamingContent&&(
                <Message msg={{id:"s",role:"assistant",content:streamingContent}} isStreaming={true} t={t}/>
              )}
              {isLoading&&!streamingContent&&(
...
function WidgetMode({t,isDark,onToggleTheme}){
  ...
  const core=useChatCore();
  const{input,setInput,attachments,addAttachment,removeAttachment,
    isLoading,streamingContent,messagesEndRef,activeConv,newConversation,sendMessage}=core;
      ...
      <div style={{position:"fixed",bottom:"100px",left:"28px",width:"360px",height:"540px",background:t.widgetBg,borderRadius:"18px",boxShadow:t.widgetShadow,display:"flex",flexDirection:"column",overflow:"hidden",zIndex:9998,transform:open?"translateY(0) scale(1)":"translateY(20px) scale(0.95)",opacity:open?1:0,pointerEvents:open?"all":"none",transition:"all 0.3s cubic-bezier(0.34,1.56,0.64,1)",border:`1px solid ${t.sideBorder}`}}>
        <div style={{padding:"12px 16px",borderBottom:`1px solid ${t.sideBorder}`,background:t.header,backdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
            ...
            <button onClick={onToggleTheme} style={{background:"none",border:`1px solid ${t.toggleBorder}`,color:t.toggleColor,borderRadius:"6px",padding:"3px 9px",cursor:"pointer",fontSize:"0.7em",fontFamily:"'Noto Kufi Arabic',sans-serif",fontWeight:600}}>
              {isDark?"☀️":"🌙"}
            </button>
          </div>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",scrollbarWidth:"thin",direction:"rtl"}}>
          {(!activeConv||activeConv.messages.length===0)&&!isLoading?(
            <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"12px",color:t.welcomeText,textAlign:"center"}}>
              ...
              <div style={{fontSize:"0.78em",opacity:0.7}}>✨ Powered by Gemini</div>
            </div>
          ):(
            <>
              {activeConv?.messages.map(msg=><Message key={msg.id} msg={msg} isStreaming={false} t={t}/>)}
